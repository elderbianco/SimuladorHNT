/**
 * Módulo de UI - Shorts Legging (REFATORADO v2)
...
function renderSizesSection() {
    ...
    // Header with Info
    const infoS = (typeof InfoSystem !== 'undefined') ? InfoSystem.getIconHTML('info_Shorts Legging_geral') : '';
...
function renderColorSection() {
    ...
    section.innerHTML = `<div class="category-header">COR DA Shorts Legging: ${cObj ? cObj.name.toUpperCase() : ''}</div>`;
...
function renderHNTLogoSection() {
    ...
    // Shared Component with Custom Logic
        ...
        },
        {
            ...
            onRenderOption: (element, color) => {
                ...
                if (baseColorHex === color.hex) {
                    ...
                    element.classList.remove('selected'); // Ensure not visually selected if disabled
                }
            }
        }
    );
...
function renderControls() {
    ...
    btnCart.onclick = () => {
        if (window.ShortsLeggingSimulatorInstance) {
            window.ShortsLeggingSimulatorInstance.handleAddToCart();
        } else {
            console.error("ShortsLeggingSimulatorInstance não encontrado");
        }
    };
    ...
    if (phoneInput && obsInput && termsCheckbox) {
        ...
        if (orderInputTop) {
            orderInputTop.onchange = (e) => {
                ...
                if (!val) {
                    state.simulationId = `HNT-SL-${suffix}`;
                } else {
                    state.simulationId = `${val}-SL-${suffix}`;
                }
