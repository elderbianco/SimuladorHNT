...
function getZoneBoundaries(element) {
    ...
    if (minX > maxX) {
        const center = zone.x;
        const radius = Math.abs(halfExpandedZoneW - halfElementWidth);
        minX = center - radius;
        maxX = center + radius;
    }
    if (minY > maxY) {
        const center = zone.y;
        const radius = Math.abs(halfExpandedZoneH - halfElementHeight);
        minY = center - radius;
        maxY = center + radius;
    }
...
async function saveOrderToHistory(silent = false, pdfUrlOverride = null) {
    ...
    if (typeof RascunhoManager !== 'undefined') {
        ...
        RascunhoManager.salvar(stateParaSalvar, pricingFull, CONFIG, pdfUrl)
            .then(res => {
                if (res.success) console.log('✅ Rascunho (Legging) salvo:', res.rascunho_id);
                else console.warn('⚠️ Rascunho (Legging) não salvo:', res.error);
            })
