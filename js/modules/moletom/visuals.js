...
function setColor(colorId) {
    ...
    if (state.logoPunho && state.logoPunho.enabled && state.logoPunho.color !== required) {
...
function updateLimits() {
    ...
    layer.innerHTML = '';
    const type = (typeof COLOR_LIMIT_MAP !== 'undefined' && COLOR_LIMIT_MAP[state.color]) ? COLOR_LIMIT_MAP[state.color] : 'dark';
    const imgs = (typeof LIMIT_IMAGES !== 'undefined') ? LIMIT_IMAGES[type] : null;

    if (imgs) {
        Object.keys(CONFIG.zones).forEach(zid => {
            if (state.zoneLimits && state.zoneLimits[zid] && imgs[zid]) {
