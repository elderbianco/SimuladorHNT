            ...
            callbacks: {
                ...
                onScale: (zoneId, val) => {
                    if (state.elements[zoneId] && state.elements[zoneId].length > 0) {
                        ...
                        // Sync to uploads state for permanent persistence
...
function renderControls() {
    ...
    btnCart.id = 'btn-add-cart'; // Reuses existing ID for listeners
    ...
    btnCart.onclick = () => {
        if (window.MoletomSimulatorInstance) {
            window.MoletomSimulatorInstance.handleAddToCart();
        } else {
            console.error("MoletomSimulatorInstance não encontrado");
        }
    };
    ...
    btnClear.style.cssText = 'flex:1; background-color: #dc3545; border-color: #dc3545; color: white; font-weight:bold; height: 100%;';
    btnClear.onclick = () => {
...
function addImage(z, s, f, is) {
    if (s) {
        // Reconstruction mode (called during loading/restoration)
