...
global.CONFIG = {
    sizes: [{label: 'M', priceMod: 0}, {label: 'GG', priceMod: 5.00}]
};

function getZonePrice(id, type) { return 0; }
...
function updatePrice() {
    ...
    updateReport(p.total, totalQty, fmtMoney(p.discountPercent), p.discountValue, p.waiver, p.devFees);
...
function calculateFullPrice() {
    ...
    // Waiver: >10 pieces AND (custom images OR customization cost) -> Waive 1 Dev Fee (or default 15?)
    // Logic from Moletom: (state.config.devFee || 15.00)
    let waiver = (state.config.artWaiver && totalQty > 10 && (customUploadsCount > 0 || customizationCost > 0)) ? (state.config.devFee || 15.00) : 0;
...
try {
    ...
    console.log('Result Top:', pricing);
